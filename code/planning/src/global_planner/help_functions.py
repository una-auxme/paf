...
def unit_vector(vector: Tuple[float, float], size: float) -> Tuple[float, float]:
    """
...
def perpendicular_vector_right(vector: Tuple[float, float]) -> Tuple[float, float]:
    """
...
def perpendicular_vector_left(vector: Tuple[float, float]) -> Tuple[float, float]:
    """
...
def scale_vector(vector: Tuple[float, float], new_len: float) -> Tuple[float, float]:
    ...
    if old_len == 0:
        return (0, 0)
    scaled_vector = (vector[0] * new_len / old_len, vector[1] * new_len / old_len)
...
def end_of_circular_arc(
    start_point: Tuple[float, float], angle: float, length: float, radius: float
) -> Tuple[float, float]:
    """
...
def circular_interpolation(
    start: Tuple[float, float], end: Tuple[float, float], arc_radius: float
):
    """
    ...
    points = [
        add_vector(circle_center, rotate_vector(vec, rot * sign)) for rot in rot_angles
    ]
...
def linear_interpolation(
    ...
    step_vector = (
        vector[0] / steps if exceeds_interval else vector[0],
