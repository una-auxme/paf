...
class UnstuckRoutine(py_trees.behaviour.Behaviour):
    ...
    def initialise(self):
        ...
        if self.current_speed.speed >= TRIGGER_STUCK_SPEED: # or target_speed.data < TRIGGER_STUCK_SPEED:
        ...
        if (curr_behavior is not None) and (curr_behavior.data == bs.int_wait.name):
            # reset stuck timer
            self.stuck_timer = rospy.Time.now()
            rospy.loginfo("WAITING at intersection, reset stuck timer")
        ...
        if curr_behavior is not None:
           if curr_behavior.data == bs.int_wait.name:
                rospy.loginfo("WAITING at intersection traffic light, no unstuck attempt")
                self.stuck_timer = rospy.Time.now()
