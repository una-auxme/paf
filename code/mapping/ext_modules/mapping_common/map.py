...
class MapTree:
    ...
    **IMPORTANT**: The map this tree was created with
    ...
    def get_entity_in_front_or_back(self, in_front=True) -> Optional[ShapelyEntity]:
        ...
        road_area = project_plane((0, -0.7), length, 1.4)
...
def build_centered_trajectory(
    ...
    hero_pt = Point2.from_vector(global_hero_transform.translation())
    hero_pt_s = shapely.Point(hero_pt.x(), hero_pt.y())
    hero_dist = global_line.line_locate_point(other=hero_pt_s)
    hero_pt_online = global_line.line_interpolate_point(hero_dist)
    split_global_lines = shapely.ops.split(global_line, hero_pt_online)
    split_global_line: LineString = split_global_lines.geoms[
        len(split_global_lines.geoms) - 1
    ]
    if max_length is not None:
        max_length_pt = split_global_line.line_interpolate_point(max_length)
        split_global_lines = shapely.ops.split(split_global_line, max_length_pt)
        split_global_line: LineString = split_global_lines.geoms[0]
    print(split_global_lines.wkt)
    return global_hero_transform.inverse() * split_global_line


def build_centered_trajectory_shape(
    global_trajectory: NavPath,
    global_hero_transform: Transform2D,
    width: float = 1.0,
    max_length: Optional[float] = None,
