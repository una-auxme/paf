...
def pos_C_rotation():
    """
    Current State: Pure Z-axis rotation by 90 degrees
...
def old_points_array():
    """
    A single point located 10m directly ahead of the car (X=10.0)
...
def test_create_transform_matrix(pos_A, pos_B_translation, pos_C_rotation):
    """
    Verifies that the 4x4 homogeneous transformation matrix
    is created correctly from a Pose.
    """
...
def test_create_delta_matrix(pos_A, pos_B_translation, pos_C_rotation):
    """
    Verifies the Delta Matrix (dT) correctly
    encodes the compensation motion needed (T_compensation).
    """
    ...
    # Expect: dT should contain a -1.0m translation
    # to undo the car's 1.0m forward motion.
    ...
    # Expect: dT must be the inverse rotation matrix (90 degrees CW rotation)
    # to compensate the turn.
...
def test_ego_motion_compensation(
    pos_A, pos_B_translation, old_points_array, pos_C_rotation, point_on_y_axis
):
    """
    Verifies that applying the compensation matrix (dT)
    shifts the old point cloud correctly into the current sensor frame.
    """
